...
Stats *s;
{
  ...
  fprintf(fd, "QUANTILE SUMMARIES\n\n");
        for(csp = s->cstats; csp != NULL; csp = csp->next) {
           fprintf(fd, "%s\n(whole class, %d tallied):\n", csp->asgt->name, csp->tallied);
           n = sizeof(quantiles)/sizeof(q);
           for(i = 0; i < n; i++)
             scores[i] = interpolatequantile(csp->freqs, csp->tallied,
                                             quantiles[i]);
           min = scores[0];
           max = scores[n-1];
           for(unsigned int i = 0; i < sizeof(quantiles)/sizeof(q); i++) {
             fprintf(fd, "   ");
...
reporttabs(FILE *fd, Course *c)
{
        ...
        for(stp = c->roster; stp != NULL; stp = stp->cnext) {
           ...
           for(ap = c->assignments; ap != NULL; ap = ap->next) {
             ...
             /*for(nscp = stp->normscores; nscp != NULL; nscp = nscp->next) {ADDED FOR COMPOSITE
               if(nscp->asgt == ap) {
                 if(nscp->flag == VALID || nscp->subst == USENORM)
                   fprintf(fd, "%6.2f\t", nscp->grade);
                 else fprintf(fd, "***.**\t");
                 goto next;
               }
